...
public class PointsController {
    ...
    public ResponseEntity<String> addPoints(
        ...
        // --- При желании можно запретить если несколько сессий ---
        if (!sessionRegistry.isUserActive(username)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body("Session not registered. Please login again.");
        }

        try {
            String backendUrl =
                    "http://backend:8080/users/" + username + "/points/add/" + amount;
    ...
    public ResponseEntity<?> updatePoints(
            @RequestParam String username, // пользователя, чей счет редактируем
            @RequestParam int amount,
            HttpSession session
    ) {
        ...
        if (!"admin".equalsIgnoreCase(currentUser)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body("Только админ может изменять очки");
        }
